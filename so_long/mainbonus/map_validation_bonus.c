...
/*   Updated: 2023/09/12 11:12:13 by beeren           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_pec(t_data *data)
{
	...
	while (data->i < data->map_height)
	{
		...
		{
			if (data->map2[data->i][data->j] == 'P')
			{
				data->ply++;
				data->player_y = data->i;
				data->player_x = data->j;
			}
			else if (data->map2[data->i][data->j] == 'E')
				data->exitt++;
			else if (data->map2[data->i][data->j] == 'C')
				data->hollow_counter++;
			data->j++;
		}
		data->i++;
